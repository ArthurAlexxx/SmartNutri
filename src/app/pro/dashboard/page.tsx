...
import { Loader2, Users, BookCheck, MoreHorizontal, Trash2 } from 'lucide-react';
...
import { doc, onSnapshot, Unsubscribe, collection, query, where, getDocs, writeBatch } from 'firebase/firestore';
...
export default function ProDashboardPage() {
  ...
  const handleDeleteTenant = async (tenantId: string, tenantName: string) => {
    if (!window.confirm(`Você tem CERTEZA que quer deletar o tenant "${tenantName}"? Esta ação é irreversível e deletará a clínica, seus profissionais e pacientes.`)) {
        return;
    }
  ...
  const renderTenantAdminDashboard = () => (
     <SummaryCards
        isProfessional={true}
        totalNutrients={{
            calorias: totalPatients, 
            proteinas: activePlans,
        }}
    />
  );
  ...
  const renderSuperAdminDashboard = () => (
    <Card>
      <CardHeader>
        <CardTitle>Visão Geral dos Clientes (Tenants)</CardTitle>
        <CardDescription>
          Gerencie e visualize todas as clínicas ativas na plataforma.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <Table>
                ...
                <TableCell>
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      <DropdownMenuItem onClick={() => handleDeleteTenant(tenant.id, tenant.name)} className="text-red-600">
